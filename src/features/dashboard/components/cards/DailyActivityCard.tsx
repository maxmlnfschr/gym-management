import { Box, Card, CardContent, Typography, Skeleton, List, ListItem, ListItemText, Divider } from '@mui/material';
              ...
              .map((checkIn, index) => {
                ...
                return (
                  <Box key={checkIn.id}>
                    <ListItem>
                      <ListItemText
                        primary={`${checkIn.member.first_name || ''} ${checkIn.member.last_name || ''}`}
                    ...
                    {index < Math.min(4, metrics.todayCheckIns.length - 1) && <Divider />}
                  </Box>
                );
              })
          ) : (
